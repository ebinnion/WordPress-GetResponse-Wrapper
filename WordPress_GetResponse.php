 ...
 * that takes advantage of core WordPress APIs.
...
class WordPress_GetResponse extends GetResponse {
	/**
	 * GetResponse API key
	...
	public function __construct($apiKey = null)
	{
		if(!extension_loaded('curl')) 
			return new WP_Error(E_USER_ERROR, 'GetResponsePHP requires PHP cURL');
		if(is_null($apiKey)) 
			return new WP_Error(E_USER_ERROR, 'API key must be supplied');
		$this->apiKey = $apiKey;
	}
	...
	public function getContactsByCustoms($campaigns = null, $customs, $operator = 'EQUALS')
	{
		...
		if(is_array($campaigns)) $params['campaigns'] = $campaigns;
		if(!is_array($customs)) 
			return new WP_Error(E_USER_ERROR, 'Second argument must be an array');
	...
	private function prepTextOp($operator, $comparison)
	{
		if(!in_array($operator, $this->textOperators)) 
			return new WP_Error(E_USER_ERROR, 'Invalid text operator');
	...
	/**
	 * Executes an API call. Overrides parent class execute method.
	 * @param string $request JSON encoded array
	 * @return object
	 * @access private
	 */
	protected function execute($request) {
		$response = wp_remote_post( $this->apiURL, array( 'body' => $request ) );
